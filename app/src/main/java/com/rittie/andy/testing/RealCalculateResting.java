            ...
            public boolean onNavigationItemSelected(MenuItem menuItem) {
                ...
                switch (menuItem.getItemId()){
                    ...
                    case R.id.nvBaselines:
                        Intent in4 = new Intent(RealCalculateResting.this, BaselineSettings.class);
                        in4.putExtra("user", u);
                        startActivity(in4);
                        return true;
        ...
        public void onBandHeartRateChanged(final BandHeartRateEvent event) {
            if (event != null) {
                if (i <= hr.length - 1) {
                    ...
                    //gets baselineID for user's "Resting" baseline
                    long baselineID = 0;
                    db.open();
                    ...
                    for (int x=0; x < hr.length; x++) {
                        db.insertHeartRate(baselineID, hr[x], startTime);
                    }
                    db.close();
                    ...
                    double averageHR = 0;
                    db.open();
                    Cursor d = db.getBaselineWithUserIDRecord(u.getId());
                    int iBaselineName2 = d.getColumnIndex(DBAdapter.BASELINE_NAME);
                    int iAverageHR2 = d.getColumnIndex(DBAdapter.AVERAGE_HEART_RATE);
                    for (d.moveToFirst(); !d.isAfterLast(); d.moveToNext()) {
                        if (d.getString(iBaselineName2).equals("Resting")) {
                            averageHR = d.getDouble(iAverageHR2);
                            break;
                        }
                    }
                    boolean b = db.updateBaselineRecord(baselineID, u.getId(), "Resting", averageHR);
                    db.close();
                    //finish();
                }
            }
